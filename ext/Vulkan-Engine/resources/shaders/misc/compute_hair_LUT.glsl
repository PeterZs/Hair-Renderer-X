...
layout( set = 0, binding = 2) uniform sampler3D NpTex;
...
// This allows to fight undersampling for low roughnesses
            ...
            const float ViewPdf   = 1.0;
                ...
                // As in the original paper "Dual scattering approximation for fast multiple-scattering in hair", the average front/back scatter are
                // cos-weighted (eq. 12).
    ...
    vec4        outColor         = vec4(saturate(FrontHemisphereOutput / float(FrontHemisphereCount) * HemisphereFactor),
                         saturate(BackHemisphereOutput / float(BackHemisphereCount) * HemisphereFactor),
                         0.0,
                         1.0);
