...
void ShadowPass::setup_attachments(std::vector<Graphics::AttachmentInfo>& attachments, std::vector<Graphics::SubPassDependency>& dependencies) {
    ...
    dependencies[0] = Graphics::SubPassDependency(STAGE_FRAGMENT_SHADER, STAGE_EARLY_FRAGMENT_TESTS, ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE);

    dependencies[0].srcAccessMask = ACCESS_SHADER_READ;

    dependencies[1] = Graphics::SubPassDependency(STAGE_LATE_FRAGMENT_TESTS, STAGE_FRAGMENT_SHADER, ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE);
